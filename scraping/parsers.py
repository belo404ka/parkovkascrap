...
__all__ =  ('work', 'rabota', 'dou', 'djinni', 'rabru')
...
def rabru(url, city=None, language=None):
    jobs = []
    errors = []
    ...
    if url:
        ...
        if resp.status_code == 200:
            soup = BS(resp.content, 'html.parser')
            all_div = soup.find('div',  attrs={'class': 'r-serp'})
            if all_div:
                hh_lst = all_div.find_all('div', attrs={'class': 'vacancy-preview-card__top'})
                for lst in hh_lst:
                    title = lst.find('h3', attrs={'class': 'vacancy-preview-card__title'})
                    href = title.a['href']
                    cont = lst.find('div', attrs={'class': 'vacancy-preview-card__short-description'})
                    content = cont.text
                    company = 'No name'
                    comp = lst.find('span',
                                   attrs={'class': 'vacancy-preview-card__company-name'})
                    if comp:
                        company = comp.text
                    jobs.append({'title': title.text, 'url': domain + href,
                                 'description': content, 'company': company,
                ...
                errors.append({'url': url, 'title': "a does not exists"})
        else:
            errors.append({'url': url, 'title': "Page do not response"})
...
if __name__ == '__main__':
    url = 'https://www.rabota.ru/vacancy/?query=java&.regionId=3&location.name=%D0%9C%D0%BE%D1%81%D0%BA%D0%B2%D0%B0&sort=relevance'
    jobs, errors = rabru(url)
